...
class AutoResizeTextView extends AppCompatTextView {
    ...
    int getTextSizeStrategy() {
        return binarySearch((int) _minTextSize, (int) _maxTextSize);
    }

    private void superSetTextSize(int startSize) {
        int textSize = getTextSizeStrategy();
        super.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
    }

    final int testTextSize(int size) {
        return _sizeTester.onTestSize(size, _availableSpaceRect);
    }
    ...
    private int binarySearch(final int start, final int end) {
        ...
        while (lo <= hi) {
            mid = lo + hi >>> 1;
            final int midValCmp = testTextSize(mid);
            if (midValCmp < 0) {
                lastBest = lo;
                lo = mid + 1;
            } else if (midValCmp > 0) {
